...
func (s *AuthService) VerifyOTP(ctx context.Context, phone, otp string) (accessToken, refreshToken, workflowID string, err error) {
	...
	captainID, err := s.findOrCreateCaptain(ctx, phone)
	if err != nil {
		return "", "", "", err
	}
	...
	wfID, err := s.repo.StartVerificationWorkflow(ctx, captainID)
	if err != nil {
		return "", "", "", err
	}

	return accessToken, refreshToken, wfID, nil
}
...
func (s *AuthService) findOrCreateCaptain(ctx context.Context, phone string) (int32, error) {

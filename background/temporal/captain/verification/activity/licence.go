...
func (a *ActivityVerfication) VerifyLicense(ctx context.Context, captainID int32) (bool, error) {
	...
	_, err = a.conn.Query.UpdateLicenseStatus(ctx, pgdb.UpdateLicenseStatusParams{
	...
	if err != nil {
		fmt.Printf("Failed to update License status: %v\n", err)
		return false, err
	}
	fmt.Printf("Marked License as verified for captain %d (VerificationID: %d)\n", captainID, verification.ID)
	return true, nil
}
